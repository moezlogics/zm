...
/**
 * Collection SEO + content widget.
 *
 * Persists three fields on `collection.metadata`:
 *   - meta_title         → <title> on the storefront collection page
 *                          (falls back to collection.title)
 ...
 *
 * Sister widget to `category-seo-widget.tsx` (same structure).
 */
...
async function fetchCollection(id: string): Promise<{
  ...
  const m = (collection?.metadata || {}) as Record<string, any>
  return {
    meta: {
    ...
    <Container className="p-4">
      <div style={{ marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 12, color: A.fgMuted, marginTop: 4 }}>
          ...
          this collection's page. The rich content block renders below
          the product grid.
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div>
          ...
          <Input
            placeholder="Leave empty to use the collection title"
            value={metaTitle}
        ...
        <div>
          ...
          <p style={{ ...adminHelpText, marginTop: 2, marginBottom: 8 }}>
            Renders below the product grid on the collection page. Great
          ...
          >
            <RichEditor
              value={content}
              onChange={setContent}
              placeholder="Write a buyer's guide, FAQ, or any extra content for this collection page…"
            />
          </div>
        </div>
